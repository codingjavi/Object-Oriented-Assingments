...
public class WordSearch {
    ...
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    public void solve() {
        int intervals = NUM_PUZZLES/NUM_THREADS;
        Thread[] threads = new Thread[NUM_THREADS];
        for (int i = 0; i <= NUM_THREADS-1; i++) {
            final int threadID = i;
            final int start = i * intervals;
            final int end = (i + 1 ) * intervals;
            //intancing a new thread to THIS THREAD WILL RUN THE SOLVE METHOD using lambda
                //start: starts running the solve method in a differetn thread
            //(new Thread(() -> solve(threadID, start, end))).start();
            Thread thread = new Thread(() -> {
                solve(threadID, start, end);
            });
    ...
    public void solve(int threadID, int firstPuzzle, int lastPuzzlePlusOne) {
        ...
        for(int i=firstPuzzle; i<lastPuzzlePlusOne; ++i) {
            //to verify no data corruption use synchronized to ensure only one thread will be using that method
            Puzzle p;
            //only 1 thread at a time can access the get method
            ...
            for(String word : p.getWords()) {
                try {
                    ...
                    if(s == null) System.err.println("#### Failed to solve " + p.name() + " for '" + word + "'");
                        ...
                        //ensure only 1 thread can use the .add method at a time
